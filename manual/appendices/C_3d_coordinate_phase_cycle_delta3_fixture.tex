...
\section{3D coordinate-residue \texorpdfstring{$\delta_3$}{delta3} presentation specification}\label{manual:app:coordinate-phase-cycle-delta3}
This appendix records a downstream coordinate-residue presentation used by time-ordered trajectory records. It gives the coordinate-level specification: integerized trajectory bins, squared displacement counts, residue-class counts, octant counts, integer ratios, and exact \(\delta_3\) residuals. It does not generate the native closure count and is not the native phase-cycle relation. Continuous-motion summaries are downstream presentation maps and do not enter the exact fixture.
The run-tumble tracer-current fixture in \secref{manual:run-tumble-tracer-current-fixture} is an application of this coordinate-residue specification after its mode, accumulated trace, and projection declarations are fixed. Existing source paths retain their legacy phase-cycle alias.

\begin{figure}[H]
\centering
\includegraphics[width=0.95\linewidth]{figures/coordinate_phase_delta3_wireframe.png}
\caption{3D coordinate-residue \(\delta_3\) presentation. The coordinate fixture maps trajectory positions into \(\mathbb Z^3\), forms integer squared displacement \(Q=a^2+b^2+c^2\), records residues \(Q\bmod\beta_a\), counts octants, and audits integer residuals by \(\delta_3\). Continuous summaries belong after the exact fixture as presentation or external maps.}
...
\subsection{Coordinate-residue presentation}
...
\]
The coordinate-residue count is
\[
...
\]
This downstream presentation is read as: mass/size data may declare \(\beta_a\), and the trajectory tests the resulting coordinate-residue counts. The declaration of \(\beta_a\) does not derive a native closure count.
...
Inputs are
\[
\texttt{track\_id},\quad \texttt{t},\quad \texttt{x},\quad \texttt{y},\quad \texttt{z}.
\]
Optional fields are
\[
\texttt{ux},\quad \texttt{uy},\quad \texttt{uz},\quad \texttt{mass},\quad \texttt{size},\quad \texttt{beta}.
\]
Output files are
\[
\begin{gathered}
\texttt{displacement\_q\_counts.csv},\\
\texttt{phase\_cycle\_delta3.csv},\\
\texttt{octant\_delta3.csv},\\
\texttt{integer\_motion\_ratios.csv}.
\end{gathered}
\]

\subsection{Final lock}
The 3D coordinate fixture is a downstream coordinate-residue presentation. It uses \(Q=a^2+b^2+c^2\), residues \(Q\bmod\beta_a\), octant counts for three-dimensional direction, and \(\delta_3\) on integer counts. It does not generate the native closure count and is not the native phase-cycle relation. External continuous-motion summaries live after this exact presentation fixture.
