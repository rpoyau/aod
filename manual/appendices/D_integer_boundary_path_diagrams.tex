...
\subsection{Integer collision examples}
\label{appD:integer-collision-examples}

The integer collision examples instantiate the vertex grammar with exact integer predicates. These rows instantiate the retained-boundary audit grammar for integer collision updates.

\begin{table}[H]
\centering
\caption{Integer collision-vertex example rows.}
\label{tab:integer-collision-examples}
...
\caption{Outcome counts for the integer collision-vertex examples.}
\label{fig:integer-collision-outcomes}
\end{figure}

\paragraph{Reproducibility.}
The raw setup, script, exact CSV, trace CSV, figure, and tests are manual-local. They are listed in the manual artifact map and audited by the manual-local application-scaffold audit.
