# Manual artifact map

This source-only file records CSV and figure companion assets used by the manual.

## Section 02 — Worked tables
- figures remain embedded or local to section bodies as migrated

## Section 03 — Temporal conversions
- manual/data/temporal/01_recurrent_ring_summary.csv
- manual/data/temporal/02_decimal_rendering_table.csv

## Section 04 — Rest-mass witnesses and external comparisons
- manual/data/rest-mass/01_rest_mass_witnesses.csv

## Section 05 — Hydrogen shells
- manual/figures/hydrogen/01_shell_radii.png

## Section 06 — Solar and radiation
- manual/figures/solar/01_system_row.png
- manual/figures/solar/02_mercury_earth_moon.png
- manual/figures/solar/03_lensing_export.png
- manual/figures/solar/04_redshift_transfer.png

## Section 07 — Confinement
- manual/figures/confinement/01_nucleus_confinement.png

## Section 08 — Galactic, biological, and planetary
- manual/data/galactic/01_five_galaxy_comparison.csv
- manual/figures/galactic/01_five_galaxy_comparison.png
- manual/figures/galactic/02_shell_family.png
- manual/figures/bacteria/01_bacteria_ominion.png
- manual/figures/saturn/01_saturn_shell_family.png

## Section 04 — Recurrent systems
- manual/figures/recurrent/01_duon_chirality_polarization.png
- manual/figures/recurrent/02_push_pull_response.png
- manual/figures/recurrent/03_flip_events.png
