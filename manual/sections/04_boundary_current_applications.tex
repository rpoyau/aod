\section{Boundary Current Applications}
\label{manual:boundary-current-applications}
\aodnoteblock{Data-support class}{Boundary-current applications are D0 exact scoped-flux fixtures. External report maps carry separate comparison rows. Errors are Stokes residuals, moving-boundary identity residuals, current-split residuals, and sheddic-route conservation residuals.}
\aodnoteblock{Scope}{This section applies the same scoped A\(\Omega\)D pressure-resolved flux calculation to wire, fluid, plasma, shell-electromagnetic comparison, and phase-fluid SADAR records. The declared boundary selects the application case of the declared-scope calculus.}

\subsection{Shared scoped-flux setup}
\label{manual:boundary-current:shared-setup}
...
B=(\partial F,\omega).
\]
The scoped A\(\Omega\)D pressure-resolved flux is
\[
...
\qquad
\operatorname{motion}_{F,k+1}(B)=(S^W_{F,k},S^W_{F,k+1}).
\]
...
\subsection{Canonical worked substitution}
\label{manual:boundary-current:worked-substitution}
...
\subsection{Wire boundary current}
\label{manual:boundary-current:wire}
Declare
\[
...
\subsection{Fluid interface dynamics}
\label{manual:boundary-current:fluid}
Declare
\[
...
\subsection{Plasma shell dynamics}
\label{manual:boundary-current:plasma}
Declare
\[
...
\subsection{Shell electromagnetic comparison}
\label{manual:boundary-current:shell-em}
...
\subsection{Phase-fluid SADAR rebalancing}
\label{manual:boundary-current:phase-fluid}
Phase-fluid motion is the realized later response of the declared interface
field to its committed SADAR attention input:
\[
\operatorname{motion}_{F,k+1}(B)=(S^W_{F,k},S^W_{F,k+1}).
\]
...
\subsection{Audit}
\label{manual:boundary-current:falsification}
...
\paragraph{Trace and response typing.}
The pressure-resolved flux in these applications is \(\phi^{\mathrm{AOD}}=p^D A\).
Normalized AFC memory/kernel flux remains separately typed. Pressure, signed
phase bias, directional RCD and current are distinct committed coordinates.
Their response changes state at the next transition; the attention vector
alone does not constitute motion. Opposite oriented values can cancel while
both bound histories remain retained.
