...
\section{Pen-and-Paper AFC Raw Execution and D.E.C. Ledger}
\label{manual:dec-ledger}
...
\subsection{D.E.C. row format}
\label{manual:dec-ledger:row-format}
...
\subsection{One-node D.E.C. ledger}
\label{manual:dec-ledger:one-node}
...
\subsection{Exact kernel normalization}
\label{manual:dec-ledger:kernel}
...
\subsection{Blocking rule}
\label{manual:dec-ledger:blocking-rule}
...
continuation trace & repeated admitted node updates after hinge slide\\
\bottomrule
\end{tabular}
...
\label{manual:dec-ledger:hinge-slide}
In the following block event, the prior hinge-state row \((e_3,0)\) attempts same-resolution dwell, but the declared scope sets \(\operatorname{adm}(e_3,0;B)=0\). The additional-dwell row \((e_3,0_D)\), not the edge slot \(e_3\) itself, is removed before renormalization; the \(e_3\) slot may re-enter only as an admitted branch state. Declare the post-block slide pool
\[
\{(e_1,-1),(e_2,-1),(e_3,+1)\}
\]
with weights
...
\label{manual:dec-ledger:field-tunnelling}
...
\label{manual:dec-ledger:adar-sadar}
...
\label{manual:dec-ledger:boundary-audit}
Declare
\[
P^D=27,
\qquad
...
\subsection{D.E.C. audit invariants}
\label{manual:dec-ledger:audit-hooks}
A D.E.C. row records
\[
B,
\quad
...
\subsection{Tau missing-burden exact ratio}
\label{manual:dec-ledger:tau-ratio}
...
\paragraph{Atomic leg and hinge role.}
Each D.E.C. row records the H1 base traversal separately from the ternary role.
Removing optional dwell sets \(m=0\) in \(\varepsilon0_H(0_D)^m(-\varepsilon)\);
the structural hinge remains. A slide pool is entered only after that hinge.
Kernel normalization uses already admissible support. Repeated updates are
continuation traces until the actual closure and distinct-reclosure witnesses
establish wave admission. The short-window examples declare neutral reflection,
the native count duration interface and a same-domain window. They clip only
\(\rho^D=\operatorname{dur}(RD\bowtie_B R^{\mathrm{asym}})\), leaving RD and RCD intact.
