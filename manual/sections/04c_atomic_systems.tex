...
Item & Declaration \\
\midrule
Parent field & cited atomic AO field \\
...
Validity window & cited shell record \\
...
A cited hydrogen shell record is fixed first. Familiar hydrogen radii are attached afterward as a local shell report on the same shell record \cite{aod43_main_2026}.
...
\begin{table}[H]
\centering
\caption{Hydrogen shell radii comparison attached after the cited shell record.}
\label{tab:hydrogen-shell-radii}
\begin{threeparttable}
\small
\begin{tabularx}{\linewidth}{@{}llcccc@{}}
\toprule
Map & Subfamily & Calculated radius [m] & Cited benchmark [m] & Abs. error [m] & Rel. error [\%] \\
\midrule
...
\bottomrule
\end{tabularx}
\ManualTableNote{The calculated and reference radii are shown only after the cited shell record is fixed. Measurement-facing SI length renderings use the cited SI/constant layer~\cite{bipm_si_brochure_2025,nist_constants_complete}.}
\end{threeparttable}
\end{table}
...
The cited hydrogen shell record carries the shell-band and attenuation witnesses of the main note, and the radii report is attached afterward as a local shell report.
...
Item & Declaration \\
\midrule
Parent field & cited atomic/nuclear AO field \\
...
Validity window & cited short window \\
...
A short-window confinement class is fixed first and the same cited row reports proximity, confinement, and transmutation status.
...
\bottomrule
\end{tabularx}
\ManualTableNote{The same cited short row resolves shell signature, dominant rung, proximity, confinement, and transmutation status.}
\end{threeparttable}
\end{table}
...
The cited short row carries the proximity, confinement, and transmutation witnesses on one row.
