...
Item & Declaration \\
\midrule
Parent field & declared five-galaxy AO field family \\
...
Validity window & declared SPARC comparison window \\
...
The declared five-galaxy set is fixed first. The outer-orbital comparison is a source-normalized presentation of this witness layer. The public SPARC source is referenced for the rotation-curve database and source quantities~\cite{sparc_paper,sparc_site}; a local snapshot used by \(\Phi_{\mathrm{SPARC}}\) must record its source units, version/access date, and normalization map before the calculated column is complete.

\begin{table}[H]
\centering
\caption{Five-galaxy SPARC declared row.}
...
\bottomrule
\end{tabularx}
\ManualTableNote{The shell-family and outer-orbital comparison are read on the same declared five-galaxy row.}
\end{threeparttable}
\end{table}
...
\toprule
Map & Galaxy & Calculated [km/s] & Declared benchmark [km/s] & Abs. error [km/s] & Rel. error [\%] \\
\midrule
...
The declared long-window record carries the shell-family witness together with the source-normalized five-galaxy comparison.
