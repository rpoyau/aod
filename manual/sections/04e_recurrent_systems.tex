...
Item & Declaration \\
\midrule
Parent field & declared recurrent AO field \\
...
Validity window & declared recurrent row \\
...
A declared recurrent-ring row is fixed first and then displayed as a compact recurrent-system witness.
...
\ManualTableNote{This compact display reuses the declared recurrent-ring row as a recurrent-system witness across scales.}
\end{threeparttable}
\end{table}
...
\begin{tikzpicture}[x=1cm,y=1cm, every node/.style={font=\small}]
  ...
  \node at (0,-1.7) {projected recurrent ring on the declared row};
\end{tikzpicture}
\caption{Recurrent ring witness.}
\label{fig:recurrent-ring-witness}
\end{figure}

The declared recurrent row carries the CK-mismatch and cut-residual diagnostics used to distinguish recurrent projection effects from kernel bias.
...
\bottomrule
\end{tabularx}
\ManualTableNote{This toy is a support row for recurrent-field diagnostics. Theorem evidence and cross-scale benchmark witnesses require their own exact rows and report maps.}
\end{threeparttable}
\end{table}

This support-row toy tracks recurrent-field diagnostics. The companion notebook tracks chirality flips, push/pull response, and mean chirality polarization under unbiased and driven reclosure schedules.
...
\midrule
Neutral external bias & 106051 & 808 & 0.00762 & 0.24 & 0.0550 \\
