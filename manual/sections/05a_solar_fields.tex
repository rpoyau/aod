% Traceability: manual-local solar field chapter.
...
The declared 100-year solar-system record is fixed first and the perihelion witness is read as an extracted Mercury slice on that same long-window record.

\begin{table}[H]
\centering
\caption{Solar perihelion declared row.}
...
\bottomrule
\end{tabularx}
\ManualTableNote{The declared row fixes the shared long-window record, including the nested Earth--Moon subfamily retained inside the same system record.}
\end{threeparttable}
\end{table}

\begin{table}[H]
\centering
\caption{Solar-system manifest on the declared 100-year row.}
\label{tab:solar-system-manifest}
\begin{threeparttable}
\small
\begin{tabularx}{\linewidth}{@{}lccc@{}}
\toprule
Subfamily / slice & Monon-period witness & Unit & Cycle count in 100 y \\
\midrule
...
\begin{table}[H]
\centering
\caption{Solar perihelion comparison attached after the declared row.}
\label{tab:solar-perihelion-comparison}
\begin{threeparttable}
\small
\begin{tabularx}{\linewidth}{@{}lcccc@{}}
\toprule
Map & Calculated [arcsec/cy] & Declared benchmark [arcsec/cy] & Abs. error [arcsec/cy] & Rel. error [\%] \\
\midrule
\(\Phi_{\varpi}\) & pending & 42.98 & pending & pending \\
\bottomrule
\end{tabularx}
\ManualTableNote{The perihelion comparison is attached only after the declared 100-year row is fixed.}
\end{threeparttable}
\end{table}

\begin{figure}[H]
\centering
\includegraphics[width=0.72\linewidth]{figures/solar/01_system_row.png}
\caption{100-year solar-system record schematic.}
\label{fig:solar-system-record}
\end{figure}

\begin{figure}[H]
\centering
\includegraphics[width=0.88\linewidth]{figures/solar/02_mercury_earth_moon.png}
\caption{Mercury perihelion slice and nested Earth--Moon subfamily.}
\label{fig:mercury-earth-moon}
\end{figure}

The long-window record carries the Mercury slice together with the cadence-support and residual-support witnesses of the main note.
...
First refinement & full source--receiver transfer row \\
Validity window & declared solar export window \\
...
The declared solar export slice is fixed first and the grazing-incidence comparison is attached afterward.

\begin{table}[H]
\centering
\caption{Solar lensing declared row.}
...
\bottomrule
\end{tabularx}
\ManualTableNote{The packet witness and shell qualification are read first on the same boundary row.}
\end{threeparttable}
\end{table}

\begin{table}[H]
\centering
\caption{Solar lensing comparison attached after the declared row.}
\label{tab:solar-lensing-comparison}
\begin{threeparttable}
\small
\begin{tabularx}{\linewidth}{@{}lcccc@{}}
\toprule
Map & Calculated [arcsec] & Declared benchmark [arcsec] & Abs. error [arcsec] & Rel. error [\%] \\
\midrule
\(\Phi_{\mathrm{lens}}\) & pending & 1.75 & pending & pending \\
\bottomrule
\end{tabularx}
\ManualTableNote{The grazing-incidence benchmark is attached after the declared export row is fixed.}
\end{threeparttable}
\end{table}
...
The declared export row carries the shell/export and boundary-deflection witnesses used for the reported grazing-incidence comparison.
...
First refinement & multi-window transfer row \\
Validity window & declared source/receiver windows \\
Residual witness & pending until refinement row is computed \\
\bottomrule
\end{tabularx}
\end{threeparttable}
...
The declared source and receiver windows are fixed first and the redshift comparison is attached afterward as a two-window transfer witness.

\begin{table}[H]
\centering
\caption{Solar redshift declared row.}
...
\bottomrule
\end{tabularx}
\ManualTableNote{The source and receiver rows are fixed first. The burden difference and cadence-transfer comparison are attached afterward across the same two-window record.}
\end{threeparttable}
\end{table}

\begin{table}[H]
\centering
\caption{Solar redshift comparison attached after the declared rows.}
\label{tab:solar-redshift-comparison}
\begin{threeparttable}
\small
\begin{tabularx}{\linewidth}{@{}lcccc@{}}
\toprule
Map & Calculated & Declared benchmark & Abs. error & Rel. error [\%] \\
\midrule
\(\Phi_z\) & pending & $2.12\times10^{-6}$ & pending & pending \\
\bottomrule
\end{tabularx}
\ManualTableNote{The redshift witness is attached after the source and receiver rows are fixed on the same two-window record.}
\end{threeparttable}
\end{table}
...
The two-window record carries the cadence-transfer witness used for the reported redshift comparison.
...
Item & Declaration \\
\midrule
Parent field & solar \(n\)-soma orbital record \\
Retained extraction & Saturn ring--moon shell family \\
Omitted context & non-Saturn solar context \\
First refinement & outer solar shell-family row \\
Validity window & declared Saturn orbital shell window \\
...
The declared Saturn ring family and selected moon shells are fixed first and then reported as one shell-family witness.

\begin{table}[H]
\centering
\caption{Saturn ring-and-moon declared row.}
...
\bottomrule
\end{tabularx}
\ManualTableNote{The declared row records the shell family and the Cassini Division gap on the same ordered orbital record.}
\end{threeparttable}
\end{table}
...
The ring family and selected moons display a readable multi-shell witness on the same orbital record.
