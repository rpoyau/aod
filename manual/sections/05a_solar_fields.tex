...
The cited 100-year solar-system record is fixed first and the perihelion witness is read as an extracted Mercury slice on that same long-window record.
...
\bottomrule
\end{tabularx}
\ManualTableNote{The cited row fixes the shared long-window record, including the nested Earth--Moon subfamily retained inside the same system record.}
\end{threeparttable}
\end{table}

\begin{table}[H]
\centering
\caption{Solar-system manifest on the cited 100-year row.}
...
The cited export row carries the shell/export and boundary-deflection witnesses used for the reported grazing-incidence comparison.
...
\bottomrule
\end{tabularx}
\ManualTableNote{The cited row records the shell family and the Cassini Division gap on the same ordered orbital record.}
\end{threeparttable}
\end{table}
