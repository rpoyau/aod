...
\subsection{3D Brownian coordinate-residue \texorpdfstring{$\delta_3$}{delta3} presentation}\label{manual:brownian-phase-delta3-fixture}
...
\]
Mass and size supply a candidate monon-beat scale \(\beta_a\). The trajectory is tested by displacement-bin counts, coordinate-residue counts \(Q_i^{(n)}\bmod\beta_a\), octant direction counts, integer ratios, and exact \(\delta_3\) residuals. The full 3D coordinate-residue presentation is given in \appref{manual:app:coordinate-phase-cycle-delta3}; its source paths retain the legacy phase-cycle alias.
...
\end{gathered}
\]
These Brownian application artifacts are legacy phase-cycle aliases of the generic 3D coordinate-residue presentation specified in \appref{manual:app:coordinate-phase-cycle-delta3}.}
\aodremark{Active coordinate: integerized three-dimensional displacement and coordinate-residue count data,
\[
z_i\in\mathbb Z^3
\to \Delta z_i^{(n)}
\to Q_i^{(n)}
...
reports & coordinate-residue counts, octant counts, integer ratios, \(\delta_3\) \\
\bottomrule
\end{tabularx}
\end{table}
