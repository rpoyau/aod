...
L1 & radial lens-medium 2D data fixture & radial medium proxies without image targets & medium-class relations & medium tallies\\
...
A field-dynamics data/projection record declares its data-support and uncertainty budget before residuals are interpreted. The budget records the declared distinction family, support class, uncertainty component, and report target.
...
\subsubsection{Run-tumble tracer-current fixture}\label{manual:run-tumble-tracer-current-fixture}
...
\subsection{SPARC square-speed 2D observable-data fixture}\label{manual:sparc-square-speed-diagnostic}
The SPARC five-galaxy square-speed fixture is the current G0 radial shell 2D observable-data lane. This record uses SPARC radial rotation-curve rows, scores the coordinate \(U=V^2\), forms \(\sigma_U=2V^{\mathrm{obs}}\sigma_V\) from the declared data uncertainty, and records \(\Delta U=U_{\AO}-U^{\mathrm{obs}}\) inside the G0 support class.
...
\]
In the five-galaxy G0/G1 projection/readout lane, the declared primary projection map uses
\[
...
\midrule
\endhead
stellar radial support & SPARC radial model & SPARC-supported radial & radial source row & 2D observable fixture\\
...
\aodremark{The SPARC square-speed lane is a G0 radial 2D projection/readout fixture. Derived residual and score outputs, where declared, diagnose the projection; they do not reclassify the dataset. The orbital-retention fixture is the G2/G3 retention-first simulation path. Projection to \(U\) follows retention, capacity seating, and \(\delta_3\) tallies.}
