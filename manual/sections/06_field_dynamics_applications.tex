...
\aodremark{Field-dynamics simulations are finite compressions of the A\(\Omega\)D ontology. A macroscopic field is simulated by declared clustering, finite boundary windows, support families, and value maps. Output residuals are read against the declared data support: boundary placement, density resolution, unresolved time-flow distinctions, angular/radial momentum inputs, slosh windows, capacity seating, and sheddic-route declarations.}
...
\scriptsize
\begin{longtable}{p{0.08\textwidth}p{0.22\textwidth}p{0.30\textwidth}p{0.24\textwidth}p{0.12\textwidth}}
...
\aodremark{This table records which distinctions are instantiated by a manual calculation. Unresolved distinctions are outside the declared data-support class unless a projection or marginalization map is declared.}
...
\scriptsize
\begin{longtable}{p{0.18\textwidth}p{0.11\textwidth}p{0.24\textwidth}p{0.17\textwidth}p{0.15\textwidth}p{0.13\textwidth}}
\caption{Field dynamics data-support and uncertainty budget.}\label{tab:field-dynamics-data-support-budget}\\
\toprule
Distinction family & Symbol & Declared data support & Support class & Uncertainty & Report target\\
\midrule
\endfirsthead
\toprule
Distinction family & Symbol & Declared data support & Support class & Uncertainty & Report target\\
\midrule
\endhead
Density & \(\epsilon_\rho\) & \(\rho(R,z,\theta,t)\) or radial proxy & 3D / 2D / radial / proxy / unresolved & \(\sigma_\rho\) when declared & field or \(U\)\\
Velocity & \(\epsilon_v\) & \(v(R,z,\theta,t)\) or radial speed & vector / radial / proxy / unresolved & \(\sigma_v\) & current, path, or \(U\)\\
Time window & \(\epsilon_\omega\) & cadence and duration & measured / declared / proxy & \(\sigma_\omega\) & slosh / retention / path\\
Boundary & \(\epsilon_{\partial}\) & \(\partial_T R_c(t)\) & 3D / radial / proxy & \(\sigma_{\partial}\) & boundary audits\\
Capacity & \(\epsilon_C\) & \(C^{\mathrm{cap}}\) & typed / scalar / inherited / proxy & \(\sigma_C\) & retention / seating\\
Coupling & \(\epsilon_E\) & \(E^{\mathfrak S}(B)\) & full / radial / proxy / ablation cut & \(\sigma_E\) & retention / projection\\
Sheddic route & \(\epsilon_X\) & \(\operatorname{SheddicPath}\) & declared / inferred / unresolved & \(\sigma_X\) & route / slosh / pending\\
External map & \(\epsilon_K\) & \(K_{\mathrm{ext}}\) or \(\lambda\) & frozen / calibrated / undeclared & \(\sigma_{\mathrm{map}}\) & external comparison\\
\bottomrule
\end{longtable}
\normalsize

\aodremark{A scored diagnostic is interpreted only within its declared support class. Differences against a higher-resolution comparison are reported as projection or marginalization uncertainty for unresolved distinctions.}
...
\small
\begin{longtable}{p{0.19\textwidth}p{0.23\textwidth}p{0.25\textwidth}p{0.22\textwidth}}
...
\aodnoteblock{Data-support class}{This Brownian / tracer-motion fixture is a G3 time-resolved tracer-current fixture when the track declares time-ordered three-dimensional positions. A two-dimensional microscopy slice is a projected tracer-current support class or ablation cut; the unresolved axis is outside the declared support class unless a projection map declares a proxy.}
...
\aodnoteblock{Source artifacts}{The source package includes \texttt{manual/scripts/aod\_brownian\_3d\_phase\_delta3.py}, \texttt{manual/data/brownian/displacement\_q\_counts.csv}, \texttt{manual/data/brownian/phase\_cycle\_delta3.csv}, \texttt{manual/data/brownian/octant\_delta3.csv}, and \texttt{manual/data/brownian/integer\_motion\_ratios.csv}. These Brownian application artifacts are aliases of the generic 3D coordinate phase-cycle fixture specified in \appref{manual:app:coordinate-phase-cycle-delta3}.}
...
\]
When \(G^{\mathrm{early}}_{e,c,t}>0\), the fixture records early coupling. Fizz is unresolved successor motion / background-continuation jitter. Pending records retained return that is not seated over the current window. The class \(\mathrm{sheddic\_route}\) records a flip carried through a declared sheddic path.
...
\midrule
\endhead
00 & \texttt{00\_Declarations} & Freeze declarations before scoring.\\
01 & \texttt{01\_Cell\_Grid\_3D} & Declare the retained 3D cell grid.\\
02 & \texttt{02\_Input\_Density\_Fields} & Declare star/gas/core/background/current density inputs and source/status records.\\
03 & \texttt{03\_Fractal\_Range\_Max\_Field\_Support} & Compute fractal range and maximum supported field count.\\
04 & \texttt{04\_Field\_Regime} & Declare gas/fluid/fizz regime.\\
05 & \texttt{05\_ChiralFlip\_EarlyCoupling\_vs\_Fizz} & Classify flips as early coupling, fizz, pending, or sheddic route.\\
06 & \texttt{06\_Range\_Edge\_Sets} & Build range-prioritized coupling families.\\
07 & \texttt{07\_SADAR\_Coupling\_Pressure\_Velocity} & Compute SADAR coupling, pressure, and motion quantities.\\
08 & \texttt{08\_Capacity\_Seating} & Seat capacity by range and closure state.\\
09 & \texttt{09\_HingeShift\_PartialCoupling} & Record hinge shift and partial coupling.\\
10 & \texttt{10\_Slosh\_Sheddic\_Pending} & Record slosh, sheddic routing, and pending return.\\
11 & \texttt{11\_Orbital\_Retention\_Class} & Classify orbital retention before projection.\\
12 & \texttt{12\_Delta3\_Retention\_Tallies} & Record exact \(\delta_3\) retention tallies.\\
13 & \texttt{13\_U\_Projection\_Diagnostic} & Optional square-speed diagnostic after retention scoring.\\
14 & \texttt{14\_Audit} & Check declaration, retention, projection, and error layers.\\
\bottomrule
\end{longtable}
\normalsize
