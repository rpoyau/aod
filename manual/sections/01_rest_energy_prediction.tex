\section{Field Rest-Energy Records}
\label{manual:rest-energy-prediction}

\subsection{0\texorpdfstring{$|-$}{|-} scope}
\label{manual:rest:scope}
...
E_{x,\mathrm{ext}}.
\]
\aodnoteblock{Data-support class}{Rest-energy records are exact internal A\(\Omega\) field records or O0 external-map comparisons, depending on the declared measured-sector target rows. Uncertainty is scoped to the external measured-sector target and value map.}

\aodprovenance{manual:app:ao-field-fractal-properties.}

\subsection{Support notation used by the calculation records}
\label{manual:rest:support-notation}
...
\]
When this odd-window instability is retained as a route-status suffix, write \(K_{\mathrm{yro}}\). Support-enclosure names remain the even-enclosure names.

\aodprovenance{manual:app:ao-field-fractal-properties.}


\subsection{A\texorpdfstring{$\Omega$}{Omega} field-map table}
\label{manual:rest:ao-field-value-table}
The A\(\Omega\) field-map table lists the manual mapping from measured-sector names to A\(\Omega\) field supports. Field-support properties are listed in the native fixture table, Table~\ref{tab:ao-field-fractal-properties}.
...
\aodprovenance{manual:app:ao-field-fractal-properties; main:subsec:curling-curl-support-forms.}
...
\subsection{Electron external map}
\label{manual:rest:electron-external-map}
...
\subsection{Measured rest-energy target records}
\label{manual:rest:measured-comparison-analysis}
...
\centering
\scriptsize
\caption{Measured rest-energy target records.}
\begin{tabularx}{\textwidth}{l r r l l}
\toprule
Record & Target [MeV] & \(\sigma_E\) [MeV] & Unit & Source\\
\midrule
\(e\) & 0.51099895069 & \(1.6\times10^{-10}\) & MeV & NIST CODATA\\
\(\mu\) & 105.6583755 & \(2.3\times10^{-6}\) & MeV & NIST CODATA\\
\(\tau\) & 1776.86 & 0.12 & MeV & NIST / PDG target record\\
\(B^\pm\) & 5279.41 & 0.07 & MeV & PDG 2025\\
\(B^0\) & 5279.72 & 0.08 & MeV & PDG 2025\\
\(B_s^0\) & 5366.93 & 0.10 & MeV & PDG 2025\\
\(B_c^\pm\) & 6274.47 & 0.32 & MeV & PDG 2025\\
\(p\) & 938.27208943 & \(2.9\times10^{-7}\) & MeV & NIST CODATA\\
\(n\) & 939.56542194 & \(4.8\times10^{-7}\) & MeV & NIST CODATA\\
\bottomrule
\end{tabularx}
\label{tab:manual-measured-comparison-analysis}
\end{table}

\aodprovenance{manual:app:ao-field-fractal-properties; manual \appref{manual:external-comparison}.}
...
\subsection{Tritrioseptyro Higgs-support diagnostic fixture}
\label{manual:tritrioseptyro-higgs-support-fixture}
...
\]
The trace is read-only for this record. Feedback experiments use separately declared write-back maps.
...
\centering
\scriptsize
\caption{Higgs-support candidate ranking. This table carries the active trace coordinates used by the internal selection rule.}
\begin{tabular}{lrrrrr}
\toprule
Candidate & \(RD\) & \(P^D_H\) & saddle & \(Q^D_H\) & score \\
\midrule
\(3{:}3{:}7_{\mathrm{yro}}\) & 61 & 21 & \(+3/-3\) & 162 & 0.00 \\
\(3{:}2{:}7_{\mathrm{yro}}\) & 21 & 21 & \(+3/-3\) & 42 & 1.58 \\
\(3{:}3{:}9_{\mathrm{yro}}\) & 61 & 27 & \(+3/0\) & 156 & 2.25 \\
\(3{:}4{:}7_{\mathrm{yro}}\) & 137 & 21 & \(+3/-3\) & 390 & 2.40 \\
\(3{:}2{:}9_{\mathrm{yro}}\) & 21 & 27 & \(+3/0\) & 36 & 2.89 \\
\(3{:}1{:}7_{\mathrm{yro}}\) & 5 & 15 & \(-3/-9\) & 0 & 3.00 \\
\(3{:}4{:}9_{\mathrm{yro}}\) & 137 & 27 & \(+3/0\) & 384 & 3.59 \\
\(3{:}1{:}9_{\mathrm{yro}}\) & 5 & 15 & \(-9/-12\) & 0 & 4.75 \\
\bottomrule
\end{tabular}
...
The trace score is reproducible under the internal selection rule:
\[
...
\subsubsection{External mass map after candidate freeze}
\label{manual:higgs-external-map-after-freeze}
...
\aodprovenance{main:subsec:yro-odd-window-route-status; manual:app:ao-field-fractal-properties; manual:tab:manual-higgs-internal-candidates; manual:tab:manual-higgs-external-map.}
