...
ax.text(7, 8.03, 'cycle -> inner enclosure -> core family -> outer shell path', ha='center', va='center', fontsize=12, color='#444')
...
roles = [
    ...
    (4.35, 6.05, 3.1, 1.60, 'Tetron', ['default inner enclosure', 'four seats / E4']),
...
center_y = 2.4
for i, (x, label) in enumerate([(2.2,'alpha outward'),(5.0,'Omega return'),(7.8,'polychiral clocks'),(10.6,'retention path')]):
...
ax.add_patch(strip)
ax.text(7,1.05, 'Tetron is the inner enclosure. Hexon / octon / decon are outer shell paths. Polychiral shell dressing is the field outer dressing.',
