...
INCLUDE_TOP_LEVEL = ['.github', '.gitignore', '.zenodo.json', '.zenodo_doi', 'BUILD.md', 'LICENSE', 'README.md', 'appendices', 'audit_pack', 'consequences', 'examples', 'main.tex', 'notebooks', 'preamble.tex', 'refs.bib', 'requirements-ci.txt', 'scripts', 'sections']
...
def write_stats(outdir: Path, stage: Path, files: list[Path], tag: str, archive_name: str) -> None:
    ...
    with report_txt.open('w', encoding='utf-8') as fh:
        ...
        fh.write('notes=canonical source archive containing active TeX, manual-local notebooks/data/figures, audit material, workflow, and metadata\n')
