...
2\times3\times2=12
\]
variants. Duon, duad, and native phase-cycle presentation. \appref{app:cc-duon-duad-cycle}.

\subsubsection{Native phase cycle and declared extractor}\label{subsec:native-phase-declared-extractor}
Let \((s_n)_{n\ge 0}\) be engine states under the indexed operations
\(s_{n+1}=\mathcal C_n(s_n)\). A native phase cycle is the minimal closed
handed return
\begin{equation}
s_0\xrightarrow{\mathcal C_0}s_1
\xrightarrow{\mathcal C_1}\cdots
\xrightarrow{\mathcal C_{N_C-1}}s_{N_C}=s_0,
\label{eq:native-phase-cycle}
\end{equation}
with closure count
\begin{equation}
N_C=\min\left\{n>0:
s_n=s_0\ \text{and}\
\operatorname{HandRet}_C(s_0,\ldots,s_n)=1
\right\}.
\label{eq:native-phase-closure-count}
\end{equation}
For \(0\le i,j<N_C\), the native rational phase accessor is
\begin{equation}
\eta_C(s_i,s_j)
=
\frac{[j-i]_{N_C}}{N_C}
\in\mathbb Q/\mathbb Z.
\label{eq:native-phase-accessor}
\end{equation}
The state \(s_0\) is the relational zero anchor of the declared cycle.

Let \(K\) be a compatible event class fixed on \(B=(\partial_TF,\omega)\),
and let \(T\) be a finite trace with event order \(\prec\). A declared
extractor is
\begin{equation}
\kappa_{K,B,\omega}(T)
=
...
\label{eq:declared-extractor}
\end{equation}
Only the selected events \(e_i\) are ticks of this extractor.
...
\qquad
N_{p{:}q}=2\Pi_{p{:}q}+1.
\label{eq:scoped-support-closure-count}
\end{equation}
When the construction trace witnesses the native closure,
\(N_C=N_{p{:}q}\). A separately declared reflection-duration accessor has
the same arithmetic value in this fixture,
\[
RD_B^{(1)}(p{:}q)=\Pi_{p{:}q}+1+\Pi_{p{:}q},
\]
without identifying phase closure with reflection duration. Thus
the candidate single-hinge counts are
\[
N_{1{:}2}=9,
\qquad
N_{3{:}2}=21,
...
N_{3{:}4}=137.
\]
On the content-bound phase traces, only \(N_{3{:}3}=61\) and
\(N_{3{:}4}=137\) are certified here as native closure counts.
The live calculus keeps the construction trace, the declared Markov choice,
hinge slide, jitter, boundary capacity, and admission status visible.
...
\]
The retained object is \(C_f[3{:}4]_{\AO}\). The closure count and the
reflection-duration accessor are separately typed scalar accessors:
\[
C_f[3{:}4]_{\AO}\ne N_{3{:}4},
\qquad
\operatorname{type}(N_{3{:}4})
\ne
\operatorname{type}\!\left(RD^{(1)}_{\AO}\!\left(C_f[3{:}4]\right)\right).
\]
...
A cycle is the retained branch/hinge/branch package. A wave is the
SADAR-valued ordered-window presentation of that package. For a retained
cycle \(C\) on \(B=(\partial_TF,\omega)\), define
\begin{equation}
...
\end{equation}
Thus \(\operatorname{wave}_B(C)\) is the SADAR-valued ordered-window
presentation of \(C\).
...
$\downarrow$ SADAR-valued ordered-window presentation on $\omega$\\[0.25em]
...
\]
with \(|\Sigma_{\mathrm{support}}|=2\). Their scoped phase-closure counts
and common recurrence window are
\[
N_{3{:}4}=137,
\qquad
N_{3{:}3}=61,
\qquad
W_{\mathrm{common}}=\operatorname{lcm}(137,61)=8357.
\]
The common window contains \(61\) closures of \(C_{3{:}4}\) and \(137\)
closures of \(C_{3{:}3}\). Common-window closure is neither cadence nor
coupling. SADAR coupling is present only when its relation is separately
declared.
...
A polyrhythm family is a finite family of cycle-valued compressions retained
on a declared boundary/window. In the notation above, such a family is
\[
\mathcal C_{\mathfrak a,r,\Sigma}
=
\operatorname{cycle}\langle\mathfrak a,r\rangle(\Sigma).
\]
Family membership and a common window establish neither cadence nor
coupling. SADAR coupling is declared separately by \(E_{\SADARop}(B)\).
For \((u,v)\in E_{\SADARop}(B)\), the coupled contribution is
\begin{equation}
\SADARop_B(C_u,C_v)
=
