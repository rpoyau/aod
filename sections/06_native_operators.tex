...
\begin{remarkx}
On shell-qualified windows, recurrent closure and phase-locking may be read as standing wavelet configurations on the cited support. Their unresolved disagreement is reported through backlog, torsional residual, cadence shift, and the rest-channel on that same window. The cut-to-cut identity is unchanged; the effective window-level progress rate need not match branch-specific traversal duration, because part of the cited window is spent servicing $\alpha/\Omega$ mismatch through bridge and dwell burden. The dominant residual-duration class $B^*$ records the leading support class of that burden on the cited window under the declared extraction policy.
\end{remarkx}
