...
For a declared shell band or active-cut family, the cadence extractor returns completed-cycle counts $N_{\rho,k}^{\mathrm{cyc}}(\omega)$ together with the shell-signature class, shell-resolved completion periods $P_{\rho,k}(\omega)$, shell-resolved phase indices $\rho_k(\omega)$, native cycle-ratio phases
\[
...
\]
where $E_{\mathrm{tot}}$ is the declared total biz-valued rate assembled from the native loss-facing channel and any declared additional native totals on the same ledger. This is the native proxy channel assembled on the cited ledger.
\end{definition}
...
\paragraph{Remark.} Long-range radiation/export windows may be reported on the duon/duad family. The duon family is the minimal native open-seat export carrier on such windows; emitted wavelet packets on the cited boundary are later physical renderings of that native export bookkeeping.
