...
This section records the compact native operator layer required for the addendum calculus. All operators below are discrete bookkeeping on declared traces, active cuts, frames/windows, and inherited support.
...
\begin{definition}[Derived putz rest rate]
\label{def:rest-putz}
With a declared attenuation accumulator $A_B(M;\omega)$ and burden reference $B_0:=1\,\mathrm{put}$, define the derived internal rest rate by
\[
R_{\mathrm{rest}}^{(\mathrm{putz})}(M;\omega):=\frac{T_\Gamma(M;\omega)}{B_0}\,A_B(M;\omega)
\qquad [\mathrm{putz}].
\]
The primitive remains $R_{\mathrm{rest}}=u^{(\mathrm{loss})}$ in trit/put.
\end{definition}

\paragraph{Depends on.} \Cref{choice:tau-readout,def:active-dwell,def:mt,def:loss-channel,def:backlog-reconciliation}.
\paragraph{Local substructure.} Admissibility count $m_t$; forced dwell; $\Gamma_{\mathrm{loss,raw}}$; $\Gamma_{\mathrm{loss}}$; $T_\Gamma$; primitive trit/put channel; derived putz channel.
\paragraph{Derivation sketch.} From the declared gating rule and the active/dwell split, admissibility restriction yields $m_t<3$ and therefore positive loss on constrained ticks. Averaging on the cited active-cut family gives the native rest channel; the putz rate is the corresponding internal rate on the same ledger.
...
\]
By linearity of the imported AFC Stokes pairing, the same quantity has the region-side form
\[
...
\paragraph{Derivation sketch.} From the chirality-resolved boundary histories on the same active-cut family and the imported AFC Stokes pairing between boundary and region accounting.
\begin{remarkx}
...
\paragraph{Local substructure.} Cadence extractor; phase/rotor accumulators; drift witnesses; cadence spectrum; dominant return class.
...
\]
induces the support mass
\[
\Lambda_b(M;\omega):=H_B(b;M;\omega)
\]
and the dominant rung
\[
B^*(M;\omega):=\arg\max_b H_B(b;M;\omega).
\]
The hard-freeze identity
\[
b=2e+f,
\qquad f\in\mathbb Z_{\ge1},
\]
exposes the integer spine of the ladder. Under the minimal-pivot idealization $f=1$, the canonical one-way excess $e=68$ yields the dominant burden rung $b=137$.
\end{definition}
...
Using the same stack ledger $S(M;\omega)$ and burden samples $b(\kappa)$ already required to compute $B^*(M;\omega)$, define the exact return attenuation accumulator
\[
...
\]
Thus $\Lambda_b$ remains the histogram support mass, while $\widehat\Lambda$ is the stack-load exponent used in attenuation.
\end{definition}

\paragraph{Depends on.} \Cref{def:b-extraction,def:bstar-histogram,choice:hard-freeze,rem:jensen,def:cycle-typing}.
\paragraph{Local substructure.} Return-duration extraction; histogram; support mass; dominant rung; shell-signature class; Jensen discipline; hard-freeze 137 spine; attenuation accumulator.
\paragraph{Derivation sketch.} From the declared return-duration samples $b(\kappa)$ and the cited weighting/filter policy, then by algebra on the stack ledger.
...
\[
\mathrm{TTL}_{\mathrm{put}}(M;\omega)=t_{\mathrm{death}}-t_{\mathrm{birth}}.
\]
...
\subsection{Native rest-channel derivation}\label{subsec:native-rest-derivation}
\begin{definition}[Native rest-channel derivation]
\label{def:native-rest-derivation}
The native rest-facing chain is the declared operator composition
\[
m_t\rightarrow u_t^{(\mathrm{loss})}\rightarrow \Gamma_{\mathrm{loss}}\rightarrow B^*\rightarrow \widehat\Lambda\rightarrow A_B\rightarrow R_{\mathrm{rest}}\rightarrow R_{\mathrm{rest}}^{(\mathrm{putz})}.
\]
Here
\[
...
=\log_3\!\bigl(\Gamma_{\mathrm{loss}}(M;\omega)\bigr)\quad[\mathrm{trit/put}],
\]
with
\[
...
\paragraph{Local substructure.} Admissibility count; native loss channel; return-duration ledger; dominant rung; stack load; attenuation accumulator; primitive trit/put channel; derived putz rate.
...
\]
where $E_{\mathrm{tot}}$ is the declared total putz-valued rate assembled from the native rest-facing channel and any declared additional native totals on the same ledger. This is the native proxy channel assembled on the cited ledger.
\end{definition}
