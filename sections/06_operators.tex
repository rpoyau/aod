...
\label{def:rest-biz}
With a declared attenuation accumulator $A_B(M;\omega)$ and burden reference $B_0:=1\,\mathrm{bip}$, define the derived internal loss rate by
\[
R_{\mathrm{loss}}^{(\mathrm{biz})}(M;\omega):=\frac{T_\Gamma(M;\omega)}{B_0}\,A_B(M;\omega)
\qquad [\mathrm{biz}].
\]
The primitive remains $R_{\mathrm{loss}}=u^{(\mathrm{loss})}$ in trit/bip and records average clock-side service burden on the cited window.
\end{definition}
...
\paragraph{Derivation sketch.} From the declared gating rule and the active/dwell split, admissibility restriction yields $m_t<3$ and therefore positive loss on constrained ticks. Averaging on the cited cut family gives the loss channel; the biz rate is the corresponding internal rate on the same ledger.
...
For a declared motif id $M$ and cited window family $\omega$, the persistence operator records the birth tick, death tick, TTL hazard, censoring status, rupture type, and any product/by-product census reported after rupture. In bip / biz / trit units,
\[
...
\paragraph{Local substructure.} Admissibility count; loss channel; return-duration ledger; dominant rung; stack load; attenuation accumulator; primitive trit/bip channel; derived biz rate.
...
\]
where $E_{\mathrm{tot}}$ is the declared total biz-valued rate assembled from the loss-facing channel and any declared additional totals on the same ledger. This is the proxy channel assembled on the cited ledger.
\end{definition}
