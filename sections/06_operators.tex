...
This section records the compact operator layer required for the calculus. All operators below are discrete bookkeeping on declared traces, cited cuts, frames/windows, and inherited support.
...
\label{def:rest-biz}
With a declared attenuation accumulator $A_B(M;\omega)$ and burden reference $B_0:=1\,\mathrm{bip}_0$, define the derived internal loss rate by
\[
R_{\mathrm{loss}}^{(\mathrm{biz})}(M;\omega):=\frac{T_\Gamma(M;\omega)}{B_0}\,A_B(M;\omega)
\qquad [\mathrm{biz}_0].
\]
The primitive remains $R_{\mathrm{loss}}=u^{(\mathrm{loss})}$ in \(\mathrm{trit}/\mathrm{bip}_0\) and records average clock-side service burden on the cited window.
\end{definition}
...
\]
The ledgers are $(A,Z,S,E)$. Derived reporting witnesses are
\[
...
\]
This is the counting contract used on the cited ledger.
\end{definition}
...
\]
and the cycle-lag witness $\Delta\varphi_{\Omega|\alpha}(M;\omega)$ under the declared pairing policy.
\end{definition}
...
\paragraph{Derivation sketch.} From the chirality-resolved boundary histories on the same cited cut family and the AFC Stokes pairing between boundary and region accounting. Recursive phase debt service is internal to the cited coupled phase family and need not require external reflection or external collision.
\begin{remarkx}
...
\begin{definition}[Cadence extractor and shell-resolved completion reports]
...
\]
The dominant class \(B^*(M;\omega)\) is recorded in \(\mathrm{bip}_0\) at the declared working cut level. The associated reciprocal support rate is \(B^*(M;\omega)^{-1}\) in \(\mathrm{biz}_0\) when that rate is used on the same ledger. This notation fixes the reference level for the cited calculation and does not assert a bottom cut; see Appendix~\ref{app:level-relative-bip-biz}.
...
\]
is evaluated on the cited admissibility ledger. Histogram- or attenuation-level quantities such as
\[
...
\label{def:path-sum}
A finite path sum is a finite sum over declared Memory paths, with each term given by a finite product of ledger-derived factors such as admissibility counts, chirality tags, cycle-ratio factors, coupling factors, and attenuation weights.
\end{definition}
...
Push/pull collision couplings of duon/duad and tetron/tetrad classes are represented as bookkeeping on declared traces. Cut diagrams, closure diagrams, path-family diagrams, and finite diagrammatic renderings package the same ledgers in compact report form.
\end{definition}
...
\begin{remarkx}
Finite path sums and diagrammatic renderings record the same cut bookkeeping on the cited window; they are report forms of the same line. They package already-declared AO field, wavelet, boundary-current, torsional, cadence, and return-ladder ledgers.
\end{remarkx}
...
For a declared motif id $M$ and cited window family $\omega$, the persistence operator records the birth tick, death tick, TTL hazard, censoring status, rupture type, and any product/by-product census reported after rupture. In \(\mathrm{bip}_0/\mathrm{biz}_0/\mathrm{trit}\) units,
\[
...
\[
m_t\rightarrow u_t^{(\mathrm{loss})}\rightarrow \Gamma_{\mathrm{loss}}\rightarrow B^*\rightarrow \widehat\Lambda\rightarrow A_B\rightarrow R_{\mathrm{loss}}\rightarrow R_{\mathrm{loss}}^{(\mathrm{biz})}.
\]
Here
\[
...
\]
\[
R_{\mathrm{loss}}(M;\omega)=u^{(\mathrm{loss})}(M;\omega):=\frac{1}{T_{\mathrm{eval}}(M;\omega)}U^{(\mathrm{loss})}(M;\omega)
=\log_3\!\bigl(\Gamma_{\mathrm{loss}}(M;\omega)\bigr)\quad[\mathrm{trit}/\mathrm{bip}_0],
\]
with
\[
...
\paragraph{Local substructure.} Admissibility count; loss channel; return-duration ledger; dominant rung; stack load; attenuation accumulator; primitive \(\mathrm{trit}/\mathrm{bip}_0\) channel; derived \(\mathrm{biz}_0\) rate.
\paragraph{Derivation sketch.} Direct algebra on the cited admissibility and return ledgers. The zero-lag/unconstrained limit $m_t=3$ yields $R_{\mathrm{loss}}=0$ and $T_\Gamma=0$.
...
Using the normalized speed $v(\omega)$ on a cited window, define the momentum-like proxy by
\[
p_{\mathrm{prox}}(M;\omega):=E_{\mathrm{tot}}(M;\omega)\,v(\omega),
\]
where $E_{\mathrm{tot}}$ is the declared total \(\mathrm{biz}_0\)-valued rate assembled from the loss-facing channel and any declared additional totals on the same ledger. This is the proxy channel assembled on the cited ledger.
\end{definition}

\paragraph{Depends on.} \Cref{def:rest-frame-window,def:loss-channel-derivation}.
\paragraph{Local substructure.} Normalized speed; rest-frame window class; proxy channel.
...
\paragraph{Derivation sketch.} From the same boundary ledgers used for shell and export classification on the cited window.
\paragraph{Remark.} Long-range radiation/export windows may be reported on the duon/duad family. The duon family is the minimal open-seat export carrier on such windows; emitted wavelet packets on the cited boundary are later physical renderings of that export bookkeeping.
...
\paragraph{Derivation sketch.} From tick ledgers and boundary bookkeeping on the declared collision/export window. Class-pair interaction tables, when recorded, follow the same ledger and the same declared collision/export-window contract.
