...
For a cited Region $R\subseteq V$ with boundary $\partial R$, a cut is the declared inside/boundary/outside split $(R,\partial R,V\setminus R)$ on which the imported AFC identity is read.
\end{definition}

\begin{definition}[Present identity]
\label{def:present-identity}
The present identity is the AFC Stokes identity evaluated on the boundary of the cited cut.
\end{definition}
...
A declared readout map assigns a ternary symbol $\tau_t\in\{-1,0,+1\}$ to each update on the cited cut. The ternary readout is attached to that cut through the declared frames and windows; it does not redefine the cut itself.
\end{choice}
...
The declared $\tau$-ledger records, on the cited frame/window family, the ternary symbols $\tau_t\in\{-1,0,+1\}$, the active/dwell partition, and any declared symbol counts $(N_+,N_0,N_-)$. These are authoritative ledgers.
\end{definition}
...
\]
The primitive is $R_{\mathrm{loss}}=u^{(\mathrm{loss})}$ in \(\mathrm{trit}/\mathrm{bip}_0\). A derived internal rate in \(\mathrm{biz}_0\) may be reported later only after the declared attenuation/backlog step has been applied. This local loss channel records readout-side admissibility restriction on the cited cut family; by itself it does not identify the imported AFC Markov-kernel capacity.
\end{definition}
...
The AFC isotropic-ignorance posture is used on the cited cut unless an explicit local refinement is declared.
\end{choice}

\begin{choice}[Anisotropic knowledge]
\label{choice:anisotropic-knowledge}
Anisotropic knowledge is a declared local refinement of distinctions or support on the cited cut. It narrows the admissible successor family on that cut and need not, by itself, introduce dwell.
\end{choice}
...
\label{def:drift}
Drift is the reported phase or transport bias that persists across the declared cut family on a cited readout class.
\end{definition}
...
A shell-band extractor packages the cited closure/return/cycle ledger into an ordered family of reported shell bands on the same cited window.
\end{definition}

\begin{definition}[Cadence extractor]
\label{def:cadence-extractor}
A cadence extractor acts on the cited shell bands to produce completed-cycle counts, completion periods, phase indices, cycle-ratio phase reports, and drift witnesses on the same cited window.
\end{definition}
...
\label{def:phase-lag}
The chirality-resolved cycle-lag statistic is the declared difference between paired $\Omega$ and $\alpha$ cycle-ratio histories on the cited readout family. Its pairing policy must be explicit.
\end{definition}
...
Let $\mathrm{Occ}(M;\omega)$ denote the cited set of motif occurrences. With declared weights $w(\kappa)\ge 0$ (TTL weighting allowed but not required), define the residual-duration histogram
\[
...
The value \(B^*(M;\omega)\) is a support-count quantity recorded in \(\mathrm{bip}_0\) at the declared working cut level. Its reciprocal \(1/B^*(M;\omega)\), when used as a rate quantity on the same cited ledger, is recorded in \(\mathrm{biz}_0\). The subscript \(0\) marks the declared reference level, not a lowest cut; see Appendix~\ref{app:level-relative-bip-biz}.
\end{definition}

\begin{remarkx}
\label{rem:bstar-class}
The $\alpha/\Omega$ branch mismatch generates the residual-duration burden on the cited window. $B^*(M;\omega)$ is the dominant class of that burden; $\Lambda_b(M;\omega)$ is the support mass on that class.
\end{remarkx}
...
\]
It is assembled from the level index, dominant residual-duration class, winding report, and support signature on the same declared shell-band extraction policy.
\end{definition}
...
The public AO field families used below are the two-seat family \emph{duon} and the four-seat family \emph{tetron}, together with their cited closure aliases \emph{duad} and \emph{tetrad} on the same family keys, declared structural subclasses, and O-prefixed chirality tags on the same field report. Here \emph{duad} is the closed/full duon alias and \emph{tetrad} is the locked/full tetron alias on the cited window. Structural examples such as \texttt{tetron:1:2:6}, \texttt{tetron:1:2:8}, \texttt{tetron:1:2:10}, \texttt{tetron:3:3:6}, and \texttt{tetrad:3:4:6} are public identifiers on cited windows.
\end{definition}
...
\label{def:ominion}
An ominion is a declared clustered effective-body state on a cited window, carrying effective report properties such as mass/rate, charge proxy, linear momentum, angular momentum, phase state, and loss/tension witnesses.
\end{definition}

\begin{definition}[n-soma system]
\label{def:n-soma}
An n-soma is a finite family of ominions together with a shared relational ledger on the cited window.
\end{definition}
...
\label{def:polyrotor}
A polyrotor phase family is a finite indexed family of rotor-completion readouts on the same cited cut family.
\end{definition}
...
On the cited shell/window, let $P_\rho(M;\omega)\in \mathbb N_{>0}$ be the rotor-completion period, let $\rho_t(M;\omega)\in\{0,1,\dots,P_\rho(M;\omega)-1\}$ be the phase index, let
\[
...
\begin{definition}[Recursive phase debt service]
\label{def:recursive-phase-debt-service}
On a declared coupled polyrotor phase family, recursive phase debt service is the internal servicing of unresolved phase disagreement under the recursive update. It need not require external reflection or external collision. On the cited readout family it is reported through torsional tension, torsional loss, the torsional residual witness, and any forced dwell where present.
\end{definition}

\begin{definition}[Torsional residual witness]
\label{def:torsional-residual-witness}
The torsional residual witness is the noncancelling remainder of the chirality-resolved boundary-current histories on the cited readout family. Its explicit operator realization is recorded later in the operator section.
\end{definition}

\begin{definition}[Cadence]
\label{def:cadence-bstar}
Cadence is the declared local recurrence-rate report quantity on the cited readout class. The dominant residual-duration class $B^*$ is defined earlier by the declared return-duration histogram and is reused here only by reference.
\end{definition}
