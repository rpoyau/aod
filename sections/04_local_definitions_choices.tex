...
\label{def:frame}
A frame $f_t$ is the one-bip readout slice on a declared cut.
\end{definition}
...
\]
The primitive is $R_{\mathrm{loss}}=u^{(\mathrm{loss})}$ in trit/bip. A derived internal rate in biz may be reported later only after the declared attenuation/backlog step has been applied. This local loss channel records readout-side admissibility restriction on the cited cut family; by itself it does not identify the imported AFC Markov-kernel capacity.
\end{definition}
