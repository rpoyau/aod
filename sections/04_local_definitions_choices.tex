...
A declared readout map assigns a ternary symbol $\tau_t\in\{-1,0,+1\}$ to each update on the cited cut. The ternary readout is attached to that cut through the declared frames and windows while the cited cut remains fixed.
\end{choice}
...
The declared $\tau$-row records, on the cited frame/window family, the ternary symbols $\tau_t\in\{-1,0,+1\}$, the active/dwell partition, and any declared symbol counts $(N_+,N_0,N_-)$. These are row entries.
\end{definition}
...
The admissibility-loss support on a cited window is the integer row
\[
...
\]
The per-tick loss burden, accumulated burden, and cited window loss readout are
\[
...
\]
Together, \(L_{\mathrm{loss}}^{\mathbb Z}\) and \(R_{\mathrm{loss}}\) name the loss channel on the cited window: the integer row records the admissibility support, and \(R_{\mathrm{loss}}\) is the retained trit-per-\(\mathrm{bip}_0\) loss readout assembled by the declared loss map.
\end{definition}

\begin{definition}[Active-only versus total duration count]
\label{def:rate-duration-count}
A rate row uses total ticks on the cited window unless the row declares an active-only duration count and records $T_{\mathrm{act}}$ alongside $T_{\mathrm{eval}}$.
\end{definition}
...
For any cited closure witness $\kappa$ with active half-length count $L_{\pm}(\kappa)$, define the level index
\[
...
The outer enclosure token $\eta_{\mathrm{out}}$ records the outer enclosure class on the cited window. A motif id $M$ is a stable label for a detected class of occurrences under declared detector settings and perturbation tolerances.
\end{definition}
...
\label{def:drift}
Drift is the phase or transport bias that persists across the declared cut family on a cited readout class.
\end{definition}
...
An AOD row records declared cut data as exact count, sign, residue, rational-pair, cycle-pair, and incidence data on a cited window. Its entries may include integer counts, signed differences, ternary signs, modular residues, support classes, phase-cycle pairs, path-count pairs, orientation signs, boundary incidences, and topology signatures.
...
\begin{definition}[Exact path-count ratio]
\label{def:exact-path-count-ratio}
A retained transport row records step motion as an integer pair
\[
...
\Delta n_e>0,
\]
where \(\Delta s_e\) is a path-step, shell-step, boundary-hop, or topology-step count, and \(\Delta n_e\) is a duration count in \(\mathrm{bip}_{\ell}\). Orientation is recorded separately by an incidence sign.
\end{definition}
...
\right).
\]
The entries record, in order, cycle data, chirality, effective-charge counts, loss counts, torsion pair, support-count witness, attenuation exponent, AOD momentum proxy, and topology signature. This is a retained boundary-link row on a cited AO field.
\end{definition}
...
A shell-band extractor packages the cited closure/return/cycle data into an ordered family of shell-band rows on the same cited window.
\end{definition}

\begin{definition}[Cadence extractor]
\label{def:cadence-extractor}
A cadence extractor acts on the cited shell bands to produce completed-cycle counts, completion periods, phase indices, cycle-ratio phase rows, and drift witnesses on the same cited window.
\end{definition}
...
\]
A declared scalar readout is obtained by the cited scalar readout map \(\mathcal N(Q,S)\).
\end{definition}
...
The value \(B^*(M;\omega)\) is a support-count quantity recorded in \(\mathrm{bip}_0\) at the declared working cut level. Its reciprocal \(1/B^*(M;\omega)\), when used as a rate quantity on the same cited row, is recorded in \(\mathrm{biz}_0\). The subscript \(0\) marks the declared reference level; see Appendix~\ref{app:level-relative-bip-biz}.
\end{definition}
...
\]
They differ in general; equivalently,
\[
J_{\mathrm{gap}}(\omega):=
...
\]
with equality only on the degenerate uniform-admissibility cases. Any rung or attenuation summary built from $b(\kappa)$ is therefore aggregated on the declared sample set before any later compression.
\end{remarkx}
...
The public AO field families used below are the two-seat family \emph{duon} and the four-seat family \emph{tetron}, together with their cited closure aliases \emph{duad} and \emph{tetrad} on the same family keys, declared structural subclasses, and O-prefixed chirality tags on the same field row. Here \emph{duad} is the closed/full duon alias and \emph{tetrad} is the locked/full tetron alias on the cited window. Structural examples such as \texttt{tetron:1:2:6}, \texttt{tetron:1:2:8}, \texttt{tetron:1:2:10}, \texttt{tetron:3:3:6}, and \texttt{tetrad:3:4:6} are public identifiers on cited windows.
\end{definition}
...
\label{def:ominion}
An ominion is a declared clustered effective-body state on a cited window, carrying effective row properties such as mass/rate, charge proxy, linear momentum, angular momentum, phase state, and loss/tension witnesses.
\end{definition}

\begin{definition}[n-soma system]
\label{def:n-soma}
An n-soma is a finite family of ominions together with a shared relational record on the cited window.
\end{definition}
...
On a declared coupled polyrotor phase family, recursive phase debt service is the internal servicing of unresolved phase disagreement under the recursive update. On the cited readout family it is recorded through torsional tension, torsional loss, the torsional residual witness, and any forced dwell where present.
\end{definition}
...
Cadence is the declared local recurrence-rate quantity on the cited readout class. The dominant residual-duration class $B^*$ is defined earlier by the declared return-duration histogram and is reused here only by reference.
\end{definition}
...
Using the declared axis-traversal rows, define the normalized speed
\[
