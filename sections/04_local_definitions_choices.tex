...
An active cut is the declared localized cut on which the derived AFC identity is read.
\end{definition}
...
A declared readout map assigns a ternary symbol $\tau_t\in\{-1,0,+1\}$ to each update on the active cut. The ternary readout is attached to the cited cut through the declared frames and windows; it does not redefine the inherited cut itself.
\end{choice}

\begin{definition}[Frame]
\label{def:frame}
A frame $f_t$ is the one-bip readout slice on a declared active cut.
\end{definition}
...
\begin{definition}[Native loss channel and loss channel]
...
\]
The native primitive is $R_{\mathrm{loss}}=u^{(\mathrm{loss})}$ in trit/bip. A derived internal rate in biz may be reported later only after the declared attenuation/backlog step has been applied.
\end{definition}
...
A shell-band extractor packages the cited closure/return/cycle ledger into an ordered family of reported shell bands. A cadence extractor acts on those bands to produce completed-cycle counts, completion periods, phase indices, cycle-ratio phase reports, and drift witnesses on the same cited window.
\end{definition}
...
The public AO field families used below are the two-seat family \emph{duon}/\emph{duad} and the four-seat family \emph{tetron}/\emph{tetrad}, together with declared structural subclasses and O-prefixed chirality tags on the same field report. Structural examples such as \texttt{tetron:1:2:6}, \texttt{tetron:1:2:8}, \texttt{tetron:1:2:10}, \texttt{tetron:3:3:6}, and \texttt{tetrad:3:4:6} are public native identifiers on cited windows.
\end{definition}
...
\label{def:ominion}
An ominion is a declared clustered effective-body state on a cited window, carrying effective report properties such as mass/rate, charge proxy, linear momentum, angular momentum, phase state, and native loss/tension witnesses.
\end{definition}
...
\label{def:polyrotor}
A polyrotor phase family is a finite indexed family of rotor-completion readouts on the same active-cut family.
\end{definition}
...
On the cited shell/window, let $P_\rho(M;\omega)\in \mathbb N_{>0}$ be the native rotor-completion period, let $\rho_t(M;\omega)\in\{0,1,\dots,P_\rho(M;\omega)-1\}$ be the native phase index, let
\[
\varphi_t(M;\omega):=\frac{\rho_t(M;\omega)}{P_\rho(M;\omega)}\in[0,1)\cap\mathbb Q
\]
be the native cycle-ratio phase, and let $N_\rho(M;\omega)\in\mathbb Z_{\ge 0}$ be the cumulative completed-cycle count. A complete native cycle increments $N_\rho$ by one.
\end{definition}
...
\[
\mathrm{TTL}_{\mathrm{bip}}(M;\omega):=t_{\mathrm{death}}-t_{\mathrm{birth}}\in\mathbb N,
\]
