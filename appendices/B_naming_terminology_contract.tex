...
\begin{itemize}[leftmargin=1.5em]
  ...
  \item \textbf{duad}: canonical closed/full duon alias on the cited row.
  \item \textbf{tetron}: public four-seat traverser family.
  \item \textbf{tetrad}: canonical locked/full tetron alias on the cited row.
\end{itemize}
...
\end{itemize}
If named properties are attached, they remain attached to the cited row under the same naming contract.

\subsection{Nested subfamily notation}
Bracketed sublists denote clustered or satellite subfamilies inside a larger cited system; for example,
\[
...
Only the shell and ladder syntax needed by the examples and the operator rows is admitted here. This includes shell indices, rung support mass, and attenuation exponents where cited. The larger manual lineage may record further serialization layers on the same row families.

\paragraph{Shell, ladder, and index syntax.}
Shell, ladder, and index syntax record only the identifiers, support witnesses, and ladder quantities used by cited rows and operator rows. Shell-facing algebra is fixed in the row where that algebra is declared.
...
Here, \emph{frame} and \emph{window} are declared local AOD row objects. \emph{Step flux} is a one-frame quantity and \emph{window current} is the derived window-level trace term used in the cited rows.
