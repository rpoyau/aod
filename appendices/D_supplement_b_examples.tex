...
\paragraph{Temporal conversion rule.} Native rows are authoritative. When conversion is required, convert only native duration bip to second, native rate biz to hertz, and the effective speed proxy to $\beta\in[0,1]$ with $c:=1$. No angular or radian-based phase variables are used in the public conversion layer.
...
\midrule
E4 native rest-channel row & bip-to-seconds, hertz, J, kg, MeV/$c^2$ dictionaries & downstream dictionary rendering \\
...
\midrule
Familiar field/particle parity & key, cited window, $\lambda$-band, shell signature, $B^*$, $R_{\mathrm{rest}}$, $R^{(\mathrm{biz})}_{\mathrm{rest}}$, cadence, torsional residual \\
...
\subsection{Example E11D: downstream decimal rendering}\label{app:e11-dict}
\paragraph{Dictionary rendering.} After the cited native E11 row is fixed, one may record decimal renderings of the same ratios. These renderings are downstream only.
{\footnotesize
\begin{center}
...
\end{tabular}
\end{center}}
\paragraph{Native anchor.} These decimals are convenience renderings of the cited native E11 ledger and must not be used as operator premises. Here $R_{\mathrm{rest}}$ is the primitive rest-facing channel on the cited row, carrying the same value as the native loss channel on that row. No angular or radian-based phase variables enter this public conversion layer.
