\section{Application and SI parity examples}\label{app:application-si}
...
\toprule
Cited source row & Downstream rendering target & Notes \\
\midrule
...
\toprule
Application class & Required cited-row starting columns \\
\midrule
...
Laboratory / engineering parity & key, cited window, shell predicate, export witness, loss/report rates, declared dictionary map id \\
Scale-comparative demonstrations & key, shell signature, cadence spectrum, confinement witness, support signature, loss/report rates \\
\bottomrule
\end{tabularx}
\end{center}
...
\paragraph{Native anchor.} Use this rendering only with an explicit citation to the corresponding cited collision/export row and its cut residual check.
...
\paragraph{Dictionary rendering.} After the cited E11 row is fixed, one may record decimal or optics-style renderings of the same ratios. These renderings are downstream only.
{\footnotesize
\begin{center}
...
\end{tabular}
\end{center}}
\paragraph{Native anchor.} These decimals are convenience renderings of the cited E11 ledger and must not be used as operator premises.
