...
\paragraph{Temporal conversion block.} Temporal conversion is a normal chain-rule expansion on the cited native row. In the current rendered witness set, the calculated outer-orbital speed is attached from the cited SPARC tail-mean witness on each extracted slice, so the comparison table records the rendered speed witness, the cited SPARC reference speed, absolute error, and relative error for the five extracted galaxy slices.
...
\caption{Calculated and observed outer-orbital speed witnesses for the five cited SPARC galaxies. The comparison is attached only after the cited native row has been fixed.}
\end{figure}
